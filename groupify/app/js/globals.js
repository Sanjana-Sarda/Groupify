...
function InviteDropdown() {
  return (
    <div id="invite-dropdown" className="dropdown-content">
      ...
      >
        <img className="copy-img" src="/static/images/copy.png"></img>
        <span>Copy</span>
      </div>
    </div>
  );
...
function LoginButton(props) {
  if (props.redirect) {
    return (
      <a className="login-button" href={`/login?redirect=${props.redirect}`}>
        {props.text}
      </a>
    );
  }
  ...
  delete_cookie("link");
  return (
    <a className="login-button" href={link}>
      {props.text}
    </a>
  );
}
...
function MemberList(props) {
  ...
  return (
    <div className="member-container">
      ...
      <h2 className="noselect" style={{ color: "white" }}>
        Members:
      </h2>
      <div className="member-list-container custom-scrollbar">{elems}</div>
...
function Members(props) {
  ...
  return <ul className="member-list">{elems}</ul>;
}
  ...
  return (
    <div>
      {props.artists.map((val, i) => {
        ...
        return (
          <span>
            <Artist key={i} link={val.link} name={val.name} />
