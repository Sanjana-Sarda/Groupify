...
def home():
    ...
    link = 'https://accounts.spotify.com/authorize?response_type=code&client_id={}&scope={}&redirect_uri={}'.format(
...
def logged_in():
    ...
    if 'access_token' in response:
        ...
        if user not in data:
            data[user] = {'token': token, 'refresh': refresh,
                          'id': user_id, 'link': link, 'premium': premium}
        else:
        ...
        resp = make_response(
            '<script src="/static/js/globals.js"></script><script src="/static/js/loggedin.js"></script>')
...
@app.route('/taher2')
def test2_html():
    resp = render_template('test2.html')
    return resp


@app.route('/taher3')
def test3_html():
    resp = render_template('test3.html')
    return resp


@app.route('/taher4')
def test4_html():
    resp = render_template('test4.html')
    return resp
...
=======
        songs_of_all_users.append(
            fetch_playlists(sp, users[user]['link'][30:]))
        # songs_of_all_users.append(get_user_top_tracks(sp))
    ...
    if username == parties[name]['owner']:
        if user_id == parties[name]['owner_id'] or party_key == parties[name]['key']:
            resp = make_response(render_template(
                'party_owner.html', host=request.host))
            resp.set_cookie('party_key', parties[name]['key'])
            resp.set_cookie('party_id', name)
            return resp
    resp = make_response(render_template(
        'party_member.html', host=request.host, party_host=parties[name]['owner']))
    return resp
...
def leave_socket(data):
    ...
    if party:
        ...
        if party in parties:
            ...
            leave_room(party)
            emit('leave',  {'username': username, 'action': 'left',
                 'members': members, 'owner': parties[party]['owner']}, room=party)
...
def end_party(data):
    ...
    if party['key'] == key:
        ...
        emit('end', '{} has ended this party'.format(
            party['owner']), room=party_id)
...
def update(data):
    ...
    if 'user' in data:
        user = data['user']
    artists = [{'name': a['name'], 'link': a['external_urls']['spotify']}
...
def fetch_playlists(sp, username):
    ...
    df_playlists = pd.DataFrame(
        {"id": id, "name": name, "#tracks": num_tracks})
    for i, playlist in enumerate(df_playlists['id']):
        try:
            string_command = "df_{} = fetch_playlist_tracks(sp, playlist)".format(
                playlist)
            exec(string_command)
        except:
            pass
...
def fetch_tracks(sp):
    ...
    df_playlist_tracks = df_playlist_tracks.assign(
...
def fetch_playlist_tracks(sp, playlistsid):
    ...
    tracks = []
    while True:
        content = sp.playlist_tracks(
            playlistsid, fields=None, limit=100, offset=offset, market=None)
...
def fetch_audio_features_playlist(sp, playlist):
    ...
    while index < playlist.shape[0]:
        ...
        genres += list(map(itemgetter('genres'),
                       sp.artists(playlist.iloc[index:index+50, 3])['artists']))
...
def preprocess(df):
    ...
    numerical_features = ['danceability', 'acousticness',
    ...
    normalized_data_set = pd.concat([df[numerical_features].reset_index(
        drop=True), ndsmx.reset_index(drop=True)], axis=1)
    normalized_data_set['instrumentalness'] = np.where(
        (normalized_data_set.instrumentalness < (10**(-3))), 0, normalized_data_set.instrumentalness)
    thresh = 100
...
def model(df1):
    ...
    for cluster in range(n_clusters):
        ...
        final.append(df2.loc[df2['cluster'] == (
            pick_cluster(df2, n1_clusters))])
...
def pick_cluster(df2, n1_clusters):
    ...
    for cluster in range(n1_clusters):
        val = df2[df2['cluster'] == cluster +
                  1]['user_id'].value_counts(normalize=True)[0]
        if (val < p):
            p = val
            o = cluster+1
    return o
...
def create_playlist(sp, playlist_name='TestPlaylist', playlist_description='Groupify Blend'):
    ...
    playlists = sp.user_playlist_create(
        user, playlist_name, description=playlist_description)
...
def enrich_playlist(sp, username, playlist_id, playlist_tracks):
    ...
    sp.user_playlist_add_tracks(
        username, playlist_id, tracks=playlist_tracks.index.values)
>>>>>>> Stashed changes
